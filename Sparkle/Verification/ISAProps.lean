...
def Opcode.encode (opc : Opcode) : Word :=
  opc.toBitVec.zeroExtend 16 <<< 13

def encodeReg (r : RegId) (shift : Nat) : Word :=
  (BitVec.ofFin (w := 3) r).zeroExtend 16 <<< shift

/-- Encode instruction to machine code -/
def encode (instr : Instruction) : Word :=
  match instr with
  | LDI rd imm =>
      Opcode.LDI.encode ||| encodeReg rd 10 ||| imm.zeroExtend 16 <<< 2
  | ADD rd rs1 rs2 =>
      Opcode.ADD.encode ||| encodeReg rd 10 ||| encodeReg rs1 7 ||| encodeReg rs2 4
  | SUB rd rs1 rs2 =>
      Opcode.SUB.encode ||| encodeReg rd 10 ||| encodeReg rs1 7 ||| encodeReg rs2 4
  | AND rd rs1 rs2 =>
      Opcode.AND.encode ||| encodeReg rd 10 ||| encodeReg rs1 7 ||| encodeReg rs2 4
  | LD rd rs1 =>
      Opcode.LD.encode ||| encodeReg rd 10 ||| encodeReg rs1 7
  | ST rs1 rs2 =>
      Opcode.ST.encode ||| encodeReg rs1 10 ||| encodeReg rs2 7
  | BEQ rs1 rs2 offset =>
      Opcode.BEQ.encode ||| encodeReg rs1 10 ||| encodeReg rs2 7 ||| offset.zeroExtend 16
  | JMP addr =>
      Opcode.JMP.encode ||| addr.zeroExtend 16

/-- Decode machine code to instruction -/
def decode (word : Word) : Option Instruction := do
  let opc ← Opcode.fromBitVec ((word >>> 13).truncate 3)
  let rBits1 := ((word >>> 10).truncate 3).toFin
  let rBits2 := ((word >>> 7).truncate 3).toFin
  let rBits3 := ((word >>> 4).truncate 3).toFin
  match opc with
  | Opcode.LDI => some (LDI rBits1 ((word >>> 2).truncate 8))
  | Opcode.ADD => some (ADD rBits1 rBits2 rBits3)
  | Opcode.SUB => some (SUB rBits1 rBits2 rBits3)
  | Opcode.AND => some (AND rBits1 rBits2 rBits3)
  | Opcode.LD  => some (LD rBits1 rBits2)
  | Opcode.ST  => some (ST rBits1 rBits2)
  | Opcode.BEQ => some (BEQ rBits1 rBits2 (word.truncate 7))
  | Opcode.JMP => some (JMP (word.truncate 13))
...
-/
theorem regId_toFin_eq_iff (x : BitVec 3) (y : RegId) : x.toFin = y ↔ x = BitVec.ofFin y := by
  constructor
  · intro h
    ext
    exact h
...
theorem encode_decode_id (instr : Instruction) :
  ...
  cases instr <;> {
    simp only [Instruction.encode, Instruction.decode, Instruction.Opcode.fromBitVec, encodeReg, Opcode.encode, Instruction.Opcode.toBitVec]
    congr
    simp only [regId_toFin_eq_iff]
    all_goals bv_decide
  }
